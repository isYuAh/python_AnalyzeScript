    ...
    def ProceedNowPartDescription(self):
        if self.CheckHasMeaning():
            if self.MODE == "DESCRIPTION":
                if self.lineToBeProceed[:9] != "#CHOICE# " and self.lineToBeProceed[0] != " ":#未开始下一部分
                    if self.lineToBeProceed[:10] == "##SLEEP## ":
                        time.sleep(float(self.lineToBeProceed.removeprefix("##SLEEP## ")))
                    elif self.lineToBeProceed[:10] == "##ENDING##":
                        self.MODE = "ENDING"
                    else:
                        print(self.lineToBeProceed)
                elif self.lineToBeProceed[:9] == "#CHOICE# " and self.lines[self.nowline][:4*self.nowPart] == self.nowTab:
                    self.MODE="CHOICE"
                    self.thisChoice.append(self.lineToBeProceed.removeprefix("#CHOICE# "))
                else:
                    if self.MODE == "ENDING":
                        self.quit()
                ...
                if (self.nowline == len(self.lines) - 1):
                    ...
                    print("----------------------------------\n\n")
                    self.quit()
                elif len(self.lines[self.nowline]) - len(self.lines[self.nowline].lstrip()) != self.nowPart * 4:
                    time.sleep(2)
                    print("\n\n----------------------------------")
                    print("游戏结束")
                    print("----------------------------------\n\n")
